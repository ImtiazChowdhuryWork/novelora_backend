...
- [ ] **Author Dashboard (next major initiative, decided 2026-08-14):** real author accounts — any logged-in app user can become an author, or sign up directly; login via Google or email/password (reusing the existing auth endpoints, not new ones); authors get their own dashboard surface, distinct from the admin dashboard. Not yet scoped: single dashboard with role-based routing vs. a separate surface, and what an author can do day one (upload novels? view stats/earnings?) vs. deferred — resolve via `AskUserQuestion` before writing schema. Once real author accounts exist, `novels.author_name` (currently a free-text string) likely gains a real FK, which would reshape the author-moderation panel shipped above.
